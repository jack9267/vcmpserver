...
inline RT GetRet(TypeWrapper<RT>, HSQUIRRELVM v, int idx) {
	typename Temporary<RT>::type st_ret;
    st_ret = Get(TypeWrapper<RT>(), v, idx);
    SquirrelObject array = SquirrelVM::GetHolderArray().GetWeakrefVal();
    array.GetValue(SQ_HOLDER_ARRAY_RET).AttachToStackObject(idx);
...
#define SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(t) \
  template<> inline t GetRet(TypeWrapper<t>, HSQUIRRELVM v, int idx) { \
    return GetRetNoReference(TypeWrapper<t>(), v, idx); \
  }
SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(int);
SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(unsigned);
SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(FLOAT);
SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(bool);
SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(short);
SQPLUS_SPECIALIZE_GETERT_BY_NOREFERENCE(char);
