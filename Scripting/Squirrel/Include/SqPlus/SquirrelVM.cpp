...
SquirrelObject  SquirrelVM::_holder_array_ref;
...
bool SquirrelVM::Init( HSQUIRRELVM v ){
    ...
    if(!v){
        ...
        created_new = true;
        sq_setprintfunc(v,SquirrelVM::PrintFunc, NULL);
        sq_pushroottable(v);
    ...
    // prepare a reference table for holding objects
    {
        SquirrelObject root = SquirrelVM::GetRootTable();
        SquirrelObject array = root.GetValue(SQ_HOLDER_ARRAY);
        if (array.IsNull()) {
            array = SquirrelVM::CreateArray(SQ_HOLDER_ARRAY_LENGTH);
            root.SetValue(SQ_HOLDER_ARRAY, array);
            for (int i = 0; i < SQ_HOLDER_ARRAY_LENGTH; ++i) {
                array.SetValue(i, SquirrelObject());
            }
        }
        _holder_array_ref.Weakref(array);
    }
...
void SquirrelVM::Release(void)
{
    _holder_array_ref.Reset();
