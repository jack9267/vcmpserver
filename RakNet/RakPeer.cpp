...
void RakPeer::ShiftIncomingTimestamp( char *data, PlayerID playerId ) const
{
	...
	memcpy( &encodedTimestamp, data, sizeof( unsigned int ) );
	encodedTimestamp = encodedTimestamp - GetBestClockDifferential( playerId );
	memcpy( data, &encodedTimestamp, sizeof( unsigned int ) );
}
...
#ifdef _SERVER
void ProcessQueryPacket( unsigned int binaryAddress, unsigned short port, char *data, int length, int iSocket )
{
	if ( length == 4 ) return;

	if ( ( data[ 0 ] == 'V' ) && ( data[ 1 ] == 'C' ) && ( data[ 2 ] == 'M' ) && ( data[ 3 ] == 'P' ) )
	{
		if ( length >= 11 )
		{
			if ( ( length == 11 ) && ( ( data[ 10 ] == 0x69 ) ) )//|| ( data[ 10 ] == 0x72 ) ) )
			{
				// Server info
				char* szReply2 = (char*)malloc( 256 );
				unsigned int uiLen = 0;
				unsigned char ucPlayers = CPlayerPoolManager::Count();
				unsigned int uiServerNameLen = strlen( CConfigLoader::GetServerName() );
				unsigned int uiModeLen = strlen( CConfigLoader::GetGamemodeName() );
				unsigned int uiMapLen = strlen( pServerInfo.szMap );

				memset( szReply2, 0, 256 );

				// HUGE memcpy/memmove list. Seems to be the way VCMP do it in their server
				memcpy( szReply2, data, 11 );
				uiLen += 11;
				memmove( &szReply2[ uiLen ], &pServerInfo.bLocked, 1 );
				uiLen++;
				memmove( &szReply2[ uiLen ], &ucPlayers, 1 );
				uiLen += 2;
				memmove( &szReply2[ uiLen ], &pServerInfo.ucMaxPlayers, 1 );
				uiLen += 2;
				memmove( &szReply2[ uiLen ], &uiServerNameLen, 4 );
				uiLen += 4;
				memmove( &szReply2[ uiLen ], &pServerInfo.szServerName, uiServerNameLen );
				uiLen += uiServerNameLen;
				memmove( &szReply2[ uiLen ], &uiModeLen, 4 );
				uiLen += 4;
				memmove( &szReply2[ uiLen ], &pServerInfo.szMode, uiModeLen );
				uiLen += uiModeLen;
				memmove( &szReply2[ uiLen ], &uiMapLen, 4 );
				uiLen += 4;
				memmove( &szReply2[ uiLen ], &pServerInfo.szMap, uiMapLen );
				uiLen += uiMapLen;

				SocketLayer::Instance() ->SendTo( iSocket, ( char* ) szReply2, uiLen, binaryAddress, port );

				delete szReply2;
			}
			else if ( ( length == 11 ) && ( data[ 10 ] == 0x63 ) )
			{
				// Playerlist
				unsigned int uiLen = 0;
				int iScore = 0;
				unsigned char ucPlayers = CPlayerPoolManager::Count(), ucNameLen = 0;
				char szTemp[ 256 ] = { NULL };

				char* szReply2 = (char*)malloc( 2048 );
				memset( szReply2, 0, 2048 );
				...
				while ( ( uc < MAX_PLAYERS ) && ( uc1 < CPlayerPoolManager::Count() ) )
				{
					...
					if ( pPlayer )
					{
						ucNameLen = strlen( pPlayer->GetNick() );
						strncpy( szTemp, pPlayer->GetNick(), 50 );
						iScore = pPlayer->GetScore();

						memmove( &szReply2[ uiLen ], &ucNameLen, 1 );
						uiLen += 1;
						memmove( &szReply2[ uiLen ], &szTemp, ucNameLen );
						uiLen += ucNameLen;
						memmove( &szReply2[ uiLen ], &iScore, 4 );
						uiLen += 4;
			...
			else if ( ( length == 15 ) && ( data[ 10 ] == 0x70 ) )
			{
				// Ping
				char szReply[ 64 ] = { NULL };

				strncpy( szReply, data, 15 );
				SocketLayer::Instance() ->SendTo( iSocket, ( char* ) &szReply, 15, binaryAddress, port );
			}
		}
	}
}
...
{
#ifdef _SERVER
	// --- START VCMP SERVER BROWSER STUFF! --- //
	ProcessQueryPacket( binaryAddress, port, data, length, rakPeer->connectionSocket );
	// ---- END VCMP SERVER BROWSER STUFF! ---- //
#endif
							...
							&& length == sizeof( UnconnectedPingStruct ) )
						{
							...
							if ( ( unsigned char ) ( data ) [ 0 ] == ID_ADVERTISE_SYSTEM && length == sizeof( unsigned char ) )
							{
								...
								if ( ( unsigned char ) ( data ) [ 0 ] == ID_PONG && length >= sizeof( UnconnectedPingStruct ) )
								{
									...
									memcpy( ( char* ) & time, data + sizeof( unsigned char ), sizeof( unsigned int ) );
									time = RakNet::GetTime() - time;
									memcpy( packet->data + sizeof( unsigned char ), ( char* ) & time, sizeof( unsigned int ) );
