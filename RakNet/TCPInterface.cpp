...
PlayerID TCPInterface::Connect(const char* host, unsigned short remotePort, bool block)
{
	if (block)
	{
	...
	else
	{
		...
		s->playerId.binaryAddress = inet_addr(host);
		s->playerId.port=remotePort;
		s->tcpInterface=this;
		...
		{
			RakNet::OP_DELETE(s);
			failedConnectionAttempts.Push(s->playerId);
		}
		return UNASSIGNED_PLAYER_ID;
	}
}
#if defined(OPEN_SSL_CLIENT_SUPPORT)
void TCPInterface::StartSSLClient(PlayerID playerId)
{
	if (ctx==0)
	{
	...
	PlayerID *id = startSSL.WriteLock();
	*id=playerId;
	startSSL.WriteUnlock();
	unsigned index = activeSSLConnections.GetIndexOf(playerId);
	if (index==(unsigned)-1)
		activeSSLConnections.Insert(playerId);
}
bool TCPInterface::IsSSLActive(PlayerID playerId)
{
	return activeSSLConnections.GetIndexOf(playerId)!=-1;
}
#endif
...
PlayerID TCPInterface::HasCompletedConnectionAttempt(void)
{
	PlayerID sysAddr=UNASSIGNED_PLAYER_ID;
...
PlayerID TCPInterface::HasFailedConnectionAttempt(void)
{
	PlayerID sysAddr=UNASSIGNED_PLAYER_ID;
...
RAK_THREAD_DECLARATION(ConnectionAttemptLoop)
{
	...
	PlayerID playerId = s->playerId;
	TCPInterface *tcpInterface = s->tcpInterface;
	RakNet::OP_DELETE(s);

	char str1[64];
	playerId.ToString(false, str1);
	SOCKET sockfd = tcpInterface->SocketConnect(str1, playerId.port);
	if (sockfd==(SOCKET)-1)
	{
		tcpInterface->failedConnectionAttemptMutex.Lock();
		tcpInterface->failedConnectionAttempts.Push(playerId);
	...
	if (tcpInterface->threadRunning)
	{
		...
		tcpInterface->completedConnectionAttempts.Push(playerId);
...
RAK_THREAD_DECLARATION(UpdateTCPInterfaceLoop)
{
	...
	{
#if defined(OPEN_SSL_CLIENT_SUPPORT)
		playerId = sts->startSSL.ReadLock();
		if (playerId)
		{
